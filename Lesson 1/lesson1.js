...
lineTo
if there is a point in the memory, say (x,y), then lineTo(X,Y) adds
to the current line collection the [line from (x,y) to (X,Y)]. 
If there is no point in the memory, then lineTo(X,Y) does nothing to the line 
collection. 
...
stroke
stroke() does the following: it draws on the canvas the lines that are in line collection
using a current_color colored and current_width pixels wide brush.

Remarks:
There is no problem to use points that exceed the bounderies of the canvas.
An occurence of stroke does not remove what was drawn by previous occurences of it,
it paints over them.
